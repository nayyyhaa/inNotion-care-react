    ...
    <Link to={`/product/${_id}`} className="card hz-card cart-card left-text row-flex text-wrap w-80p h-20rm m-v-2">
      ...
      <div className="card-header-text cart-card-header h-25rm col-flex flex-start no-wrap">
        ...
        <div className="quantity-selector row-flex no-wrap m-b-2">
          <button className="icon-btn" onClick={(e) => removeFromCart(_id, e)}>
            <i className="fa fa-trash-o light-font m-r-2 red-text" aria-hidden="true"></i>
          </button>
          <button className="icon-btn" onClick={(e) => updateQuantityInCart(_id, "decrement", qty, e)}>
            <i className="fa fa-minus light-font m-r-2" aria-hidden="true"></i>
          </button>
          <p className="quantity m-r-2 h3">{qty}</p>

          <button className="icon-btn" onClick={(e) => updateQuantityInCart(_id, "increment", qty, e)}>
